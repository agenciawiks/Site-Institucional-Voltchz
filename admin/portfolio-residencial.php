<?php
/**
 * VoltchZ Brasil - Gerenciamento de Portfólio Residencial
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_item') {
    ...
    if ($id) {
        try {
            $stmtImg = $db->prepare("SELECT image FROM portfolio_residencial WHERE id = ?");
            $stmtImg->execute([$id]);
            $image_path = $stmtImg->fetchColumn();

            $stmt = $db->prepare("DELETE FROM portfolio_residencial WHERE id = ?");
            $stmt->execute([$id]);
            ...
            $success_message = "Item de portfólio residencial excluído com sucesso.";
        } catch (Exception $e) {
...
$sql = "SELECT * FROM portfolio_residencial WHERE 1=1";
$params = [];
...
admin_header("Gerenciar Portfólio Residencial / Veículos", "portfolio-residencial");
?>
...
<div class="bg-brand-bg2 border border-white/5 rounded-2xl p-5 mb-8">
    <form method="GET" class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <a href="portfolio-form.php?type=residencial" class="bg-brand-green hover:brightness-110 text-brand-bg font-bold px-4 py-2.5 rounded-xl flex items-center justify-center gap-1 transition-all shadow-lg shadow-brand-green/10 shrink-0">
    ...
    <?php foreach ($items as $item): ?>
        <div class="bg-brand-bg2 border border-white/5 rounded-2xl overflow-hidden group hover:border-white/10 transition-all flex flex-col justify-between">
            ...
            <div class="p-5 pt-0 border-t border-white/5 flex items-center justify-between gap-2 mt-auto">
                <a href="portfolio-form.php?type=residencial&id=<?php echo $item['id']; ?>" class="flex-1 text-center py-2 rounded-xl border border-white/5 hover:bg-white/5 text-white text-xs font-semibold transition-all">
                    Editar
                </a>
