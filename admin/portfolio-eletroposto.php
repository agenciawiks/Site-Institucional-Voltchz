<?php
/**
 * VoltchZ Brasil - Gerenciamento de Portfólio de Eletropostos / Comercial
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_item') {
    ...
    if ($id) {
        try {
            $stmtImg = $db->prepare("SELECT image FROM portfolio_eletroposto WHERE id = ?");
            $stmtImg->execute([$id]);
            $image_path = $stmtImg->fetchColumn();

            $stmt = $db->prepare("DELETE FROM portfolio_eletroposto WHERE id = ?");
            $stmt->execute([$id]);
            ...
            $success_message = "Item de eletroposto excluído com sucesso.";
        } catch (Exception $e) {
...
try {
    $stmt = $db->query("SELECT * FROM portfolio_eletroposto ORDER BY sort_order ASC, id DESC");
    $items = $stmt->fetchAll();
} catch (Exception $e) {
    $items = [];
...
admin_header("Gerenciar Portfólio Eletropostos & Comercial", "portfolio-eletroposto");
?>
...
<div class="bg-brand-bg2 border border-white/5 rounded-2xl p-5 mb-8 flex items-center justify-between">
    <div>
        <h2 class="text-white font-bold text-lg">Casos em Eletropostos e Estabelecimentos Comerciais</h2>
        <p class="text-brand-muted text-xs">Instalações em empresas, frotas, concessionárias e hubs de recarga rápida DC/AC</p>
    </div>
    <a href="portfolio-form.php?type=eletroposto" class="bg-brand-green hover:brightness-110 text-brand-bg font-bold px-4 py-2.5 rounded-xl flex items-center gap-1 transition-all shadow-lg shadow-brand-green/10 shrink-0">
    ...
    <?php foreach ($items as $item): ?>
        <div class="bg-brand-bg2 border border-white/5 rounded-2xl overflow-hidden group hover:border-white/10 transition-all flex flex-col justify-between">
            ...
            <div class="p-5 pt-0 border-t border-white/5 flex items-center justify-between gap-2 mt-auto">
                <a href="portfolio-form.php?type=eletroposto&id=<?php echo $item['id']; ?>" class="flex-1 text-center py-2 rounded-xl border border-white/5 hover:bg-white/5 text-white text-xs font-semibold transition-all">
                    Editar
                </a>
