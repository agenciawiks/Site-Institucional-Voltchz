...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_banner') {
    $banner_id = filter_input(INPUT_POST, 'id', FILTER_VALIDATE_INT);
    if ($banner_id) {
        try {
            // Busca a imagem para deleção física no disco
            $stmtImg = $db->prepare("SELECT image FROM banners WHERE id = ?");
            $stmtImg->execute([$banner_id]);
            $banner_image = $stmtImg->fetchColumn();
...
try {
    ...
    $banners = [];
    $error_message = "Erro ao buscar banners: " . $e->getMessage();
}

admin_header("Gerenciar Banners da Home", "banners");
?>
...
<div class="flex justify-between items-center bg-brand-bg2 border border-white/5 rounded-2xl p-5 mb-8">
    <span class="text-xs text-brand-muted">Ordene os banners por ordem numérica crescente. Banners marcados como inativos não serão mostrados na Home.</span>
    <a href="banners-form.php" class="bg-brand-green hover:brightness-110 text-brand-bg font-bold px-4 py-2.5 rounded-xl flex items-center justify-center gap-1 transition-all shadow-lg shadow-brand-green/10">
...
<div class="bg-brand-bg2 border border-white/5 rounded-2xl p-6">
    <?php if (empty($banners)): ?>
        <div class="text-center py-20 bg-brand-bg/30 border border-white/5 border-dashed rounded-2xl">
            ...
            <h4 class="text-white font-semibold text-sm">Nenhum banner cadastrado</h4>
            <p class="text-xs text-brand-muted/70 mt-1">Crie um novo banner no botão verde acima.</p>
        </div>
    <?php else: ?>
            ...
            <table class="w-full text-left text-sm text-brand-muted">
                <thead>
                    <tr class="border-b border-white/5 text-[11px] font-bold uppercase tracking-wider text-white bg-brand-bg/30">
                        ...
                        <th class="py-3 px-4">Texto do Banner</th>
                        <th class="py-3 px-4">Botão</th>
                    ...
                    ?>
                        <tr class="hover:bg-white/[0.01] transition-colors">
                            ...
                            <td class="py-4 px-4 max-w-xs">
                                <span class="block font-bold text-white truncate"><?php echo htmlspecialchars($b['title']); ?></span>
                                <span class="block text-xs text-brand-muted/80 truncate"><?php echo htmlspecialchars($b['subtitle']); ?></span>
                            </td>
                            <td class="py-4 px-4 text-xs font-medium">
                                <?php if (!empty($b['button_text'])): ?>
                                    <span class="block text-white"><?php echo htmlspecialchars($b['button_text']); ?></span>
                                    <span class="block text-[10px] font-mono text-brand-muted/60 truncate max-w-[120px]"><?php echo htmlspecialchars($b['button_link']); ?></span>
                                <?php else: ?>
                                    <span class="text-brand-muted/40 font-italic">Nenhum</span>
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="py-4 px-4 text-right">
                                <div class="flex justify-end items-center gap-2">
                                    <a href="banners-form.php?id=<?php echo $b['id']; ?>" class="p-2 rounded-xl bg-white/5 border border-white/5 hover:border-white/10 hover:text-white text-brand-muted transition-all" title="Editar">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"></path></svg>
                                    </a>
                                    
                                    <form method="POST" action="" onsubmit="return confirm('Tem certeza que deseja excluir este banner?');" class="inline">
