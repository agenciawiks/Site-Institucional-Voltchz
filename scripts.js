...
function initClients() {
    ...
    CONFIG.CLIENTS.forEach((id, idx) => {
        ...
        slide.style.backgroundImage = `url('IMAGENS/CLIENTES/cliente-${id}.webp')`;
        slide.innerHTML = `
      ...
      `;
        slide.addEventListener('click', () => openLightbox(`IMAGENS/CLIENTES/cliente-${id}.webp`));
...
function initLogos() {
    ...
    CONFIG.LOGOS.forEach(logo => {
        ...
        card.innerHTML = `<img src="IMAGENS/LOGOS-CLIENTES/logo-${logo.id}.${logo.ext}" alt="Parceiro ${logo.id}" class="h-10 w-auto opacity-50 group-hover:opacity-100 transition-all">`;
